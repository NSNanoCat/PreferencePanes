...
截至 2026-09-07，本地 JSON 已调整为模块 BoxJS 探测、配置加载、设置子树读取与单键操作，共 3 个路径、7 个 HTTP 操作。POST 正文为 JSON 值本身，POST/DELETE 成功均为 200；模块根由原生 Mock 提供。此次本地修改尚未推送，以下首次同步记录仅代表此前版本。example.org 为文档占位，不是已部署的服务。
