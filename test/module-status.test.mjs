...
test("late and cancelled probes cannot overwrite newer status", async t => {
    ...
    const first = status.check("/api/Module");
    const second = status.check("/api/Module");
    pending[1](new Response(null, { status: 404 }));
    await second;
    pending[0](new Response(null, { status: 200 }));
    await first;
    assert.equal(element.textContent, "未安装");
    const last = status.check("/api/Module");
    status.destroy();
...
test("probeModule returns the HTTP status for unavailable modules", async () => {
    const response = await probeModule("https://example.org/api/Module", {
...
test("probeModule preserves network failures for the caller", async () => {
    await assert.rejects(
        probeModule("https://example.org/api/Module", {
            fetch: async () => {
